"""
auth/google_auth.py — Streamlit native OIDC auth (st.login / st.user / st.logout)

THE FIX: require_auth() must call st.stop() itself after rendering the landing page.
app.py must NOT have any if/stop wrapping around require_auth().

secrets.toml must have [auth] section:
    [auth]
    redirect_uri        = "https://comp-intel-analysis-agent.streamlit.app/oauth2callback"
...
def init_session() -> dict:
    """Populate st.session_state from st.user after successful login."""
    u       = st.user
...
def _render_landing_page():
    ...
    * { font-family: 'Inter', sans-serif; box-sizing: border-box; }
    ...
    .stats-row {
        ...
        margin-bottom:40px; animation:fadeInUp .6s ease .3s both; position:relative; z-index:1;
    }
    ...
    .ticker-wrap {
        width:100%; overflow:hidden; background:rgba(255,255,255,.04);
        border-top:1px solid rgba(255,255,255,.08); border-bottom:1px solid rgba(255,255,255,.08);
        padding:14px 0; margin-top:40px; position:relative; z-index:1;
    }
    .ticker-track { display:flex; width:max-content; animation:ticker 30s linear infinite; }
    .ticker-item {
        display:inline-flex; align-items:center; gap:10px;
        font-size:13px; font-weight:500; color:#64748b; padding:0 32px; white-space:nowrap;
    }
    .ticker-item span { color:#3b82f6; font-size:16px; }
    .features-section { background:#f8f7f4; padding:96px 24px; }
    .section-label { text-align:center; font-size:11px; font-weight:700;
        letter-spacing:.1em; text-transform:uppercase; color:#1a56db; margin-bottom:16px; }
    .section-title { text-align:center; font-size:clamp(28px,4vw,44px); font-weight:800;
        color:#0f172a; letter-spacing:-.03em; line-height:1.2; margin-bottom:16px; }
    .section-sub { text-align:center; font-size:16px; color:#64748b;
    ...
    .feature-card { background:#fff; border:1px solid #e8e4dd; border-radius:16px;
        padding:32px; box-shadow:0 2px 8px rgba(0,0,0,.04); transition:transform .2s,box-shadow .2s; }
    .feature-card:hover { transform:translateY(-4px); box-shadow:0 12px 32px rgba(0,0,0,.08); }
    .feature-icon { width:48px; height:48px; border-radius:12px; font-size:22px;
    ...
    .steps-row { display:flex; gap:0; max-width:900px; margin:0 auto; flex-wrap:wrap; justify-content:center; }
    .step { flex:1; min-width:200px; text-align:center; padding:0 24px; position:relative; }
    .step:not(:last-child)::after { content:'→'; position:absolute; right:-12px; top:20px;
        font-size:20px; color:#cbd5e1; }
    .step-num { width:44px; height:44px; border-radius:50%; background:#eff6ff;
        border:2px solid #bfdbfe; color:#1a56db; font-size:16px; font-weight:800;
    ...
    .cta-section { background:linear-gradient(135deg,#0f172a 0%,#1e3a8a 100%);
        padding:80px 24px 20px; text-align:center; }
    .cta-title { font-size:clamp(28px,4vw,48px); font-weight:800; color:#fff;
        letter-spacing:-.03em; margin-bottom:16px; }
    .cta-sub { font-size:18px; color:#94a3b8; margin-bottom:40px; }
    .cta-note { font-size:11px; color:#475569; background:#0f172a; padding:0 0 32px; margin:0; text-align:center; }
    .app-footer { background:#0f172a; border-top:1px solid rgba(255,255,255,.06);
        padding:28px 24px; text-align:center; color:#475569; font-size:13px; }
    .stButton > button {
        background:#fff !important; color:#1e293b !important; border:none !important;
        border-radius:12px !important; font-size:15px !important; font-weight:600 !important;
        padding:14px 32px !important; box-shadow:0 4px 16px rgba(0,0,0,.2) !important;
    }
    .stButton > button:hover {
        transform:translateY(-2px) !important; box-shadow:0 8px 24px rgba(0,0,0,.28) !important;
    }
    </style>
    ...
    <div class="features-section">
        ...
        <div class="features-grid">
            <div class="feature-card"><div class="feature-icon" style="background:#eff6ff">🌐</div>
                <div class="feature-title">Multi-Source Scraping</div>
                <div class="feature-desc">Monitors competitor websites, blogs, product docs, changelogs, and YouTube channels automatically.</div></div>
            <div class="feature-card"><div class="feature-icon" style="background:#fdf4ff">🧠</div>
                <div class="feature-title">GPT-4o Vision Analysis</div>
                <div class="feature-desc">Reads screenshots, pricing tables, and roadmap slides from your Google Doc scrapbook.</div></div>
            <div class="feature-card"><div class="feature-icon" style="background:#f0fdf4">🔄</div>
                <div class="feature-title">Semantic Diff Engine</div>
                <div class="feature-desc">Only surfaces what actually changed since last run — no noise, just signal.</div></div>
            <div class="feature-card"><div class="feature-icon" style="background:#fff7ed">⚡</div>
                <div class="feature-title">Real-Time Streaming</div>
                <div class="feature-desc">Watch the analysis happen live with streaming progress and synthesis preview.</div></div>
            <div class="feature-card"><div class="feature-icon" style="background:#fef2f2">🔒</div>
                <div class="feature-title">Private &amp; Isolated</div>
                <div class="feature-desc">Your configurations and reports are completely private — no other user can see your data.</div></div>
            <div class="feature-card"><div class="feature-icon" style="background:#eff6ff">☁️</div>
                <div class="feature-title">Google Drive Archive</div>
                <div class="feature-desc">Publish and archive reports to Google Drive and share with your team via email.</div></div>
        </div>
    </div>
    ...
    <div class="how-section">
        ...
        <div class="steps-row">
            <div class="step"><div class="step-num">1</div>
                ...
                <div class="step-desc">Enter URLs for website, blog, docs, changelog, and YouTube</div></div>
            <div class="step"><div class="step-num">2</div>
                <div class="step-title">Ask a Question</div>
                <div class="step-desc">Type a research query like "What's new in their pricing?"</div></div>
            <div class="step"><div class="step-num">3</div>
                <div class="step-title">Agent Runs</div>
                <div class="step-desc">LangGraph pipeline scrapes, reads, and synthesizes all sources</div></div>
            <div class="step"><div class="step-num">4</div>
                <div class="step-title">Read the Report</div>
                <div class="step-desc">8-dimension analysis with delta highlights in under 60 seconds</div></div>
        </div>
    </div>
...
def require_auth():
    """
    THE CORRECT PATTERN (official Streamlit docs):

        if not st.user.is_logged_in:
            st.button("Log in", on_click=st.login)
            st.stop()   <-- THIS is what shows the page and blocks the rest

    RULES:
      1. on_click=st.login  (callback) — renders button first, login fires on click
      2. st.stop() inside this function — NOT in app.py
      3. app.py calls require_auth() with NO wrapping if/stop at all
    """
    if not st.user.is_logged_in:
        ...
        with col2:
            st.button(
                "🔑  Sign in with Google",
                on_click=st.login,           # callback — does NOT fire immediately
                type="primary",
        ...
        st.markdown(
            "<div class='app-footer'>Built with LangGraph · GPT-4o Vision · Streamlit"
            " &nbsp;|&nbsp; © 2026 CompIntel</div>",
            unsafe_allow_html=True,
        )

        # st.stop() halts script here — landing page stays visible.
        # This is THE critical line that was missing in all previous versions.
        st.stop()

    # Only reaches here when logged in
...
def render_user_pill():
    ...
    if picture:
        ...
        avatar = (
            f"<div style='width:32px;height:32px;border-radius:50%;background:#1a56db;"
