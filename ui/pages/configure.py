...
from db.auth_db import (
    get_competitors_for_user, add_competitor_for_user,
    get_competitor_by_name_for_user
)
from db.database import update_competitor, delete_competitor
...
def render():
    ...
    st.markdown(
        ...
        "Your competitor list is private — only you can see and edit it.</p>",
        ...
        with st.form("add_competitor_form", clear_on_submit=True):
            ...
            if submitted:
                if not name.strip():
                    ...
                    success = add_competitor_for_user(
                        user_id, name.strip(), website, blog, docs, changelog, youtube
                    )
                    if success:
                        st.markdown("""
                            <div style='display:flex;align-items:center;gap:8px;
                                ...
                                <span style='color:#94a3b8;font-size:13px'>— competitor added.</span>
                            </div>
                        """, unsafe_allow_html=True)
                        st.rerun()
                    else:
                        st.error(f"A competitor named '{name}' already exists in your list.")
    ...
    st.markdown(
        ...
        f"text-transform:uppercase'>Your Competitors ({len(competitors)})</p>",
        unsafe_allow_html=True
    )
